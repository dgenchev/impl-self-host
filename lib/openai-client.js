...
const ExtractedPatientSchema = z.object({
    firstName: z.string().nullable().optional().transform(v => v ?? null),
    lastName: z.string().nullable().optional().transform(v => v ?? null),
    phone: z.string().nullable().optional().transform(v => v ?? null),
    email: z.string().nullable().optional().transform(v => v ?? null),
    dentalConcerns: z.string().nullable().optional().transform(v => v ?? null),
    preferredTimes: z.string().nullable().optional().transform(v => v ?? null),
...
- Return raw JSON only, no markdown, no explanation`
    ...
    try {
        ...
        return ExtractedPatientSchema.parse(parsed);
    } catch (error) {
        console.error('Failed to parse/validate patient info extraction:', error.message);
        console.error('Raw GPT response was:', rawContent);
        return {
            firstName: null, lastName: null, phone: null, email: null,
            dentalConcerns: null, preferredTimes: null, isComplete: false
        };
    }
}
