...
function withTimeout(promise) {
    return Promise.race([
        ...
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Telegram API call timed out after 15s')), TELEGRAM_TIMEOUT_MS)
        )
    ]);
}
...
async function sendMessage(text, options = {}) {
    try {
        ...
        const response = await withTimeout(bot.sendMessage(
            chatId,
            text,
...
_Изпратено от AI асистента_`;
        ...
        const response = await withTimeout(bot.sendPhoto(chatId, fileBuffer, {
            caption,
            parse_mode: 'Markdown'
        }));
...
_Изпратено от AI асистента_`;
        ...
        const response = await withTimeout(bot.sendDocument(chatId, fileBuffer, {
            caption,
            parse_mode: 'Markdown'
        }, {
            filename: filename
